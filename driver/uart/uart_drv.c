...
void uart3_isr(void) interrupt UART3_VECTOR
{
	...
	if (S3CON & S3TI) { //发送中断
		...
		if (trans3_ctr < trans3_size) { //未传送完成
			...
			nop_num = 100;
			while (nop_num--);
			bRS485_3_Ctrl = 0;	        //禁止发送, 转为接收
			UART3_RECEIVE_ENABLE();  //UART3允许接收
		}
...
void uart4_isr(void) interrupt UART4_VECTOR
{
	...
	if (S4CON & S4TI) { //发送中断
		...
		if (trans4_ctr < trans4_size) { //未传送完成
			...
			nop_num = 100;
			while (nop_num--);
			bRS485_4_Ctrl = 0;	         //禁止发送, 转为接收
			UART4_RECEIVE_ENABLE();  //UART4允许接收
		}
